...
const { Field, FieldFail } = require('./field/base');
...
class Form {
  ...
  async init(init, data) {
    ...
    for (const [ name, option ] of Object.entries(init.fields)) {
      ...
      const opt = Object.assign({}, option instanceof Field ? await option.build() : option);
        ...
        try {
          ...
          if (value !== undefined) {
            ...
            if (option instanceof Field) {
              await option.postValidate(value);
            }
          }
        } catch (e) {
          this[ERRORS][name] = e;
  ...
  get errorMessages() {
    ...
    Object.entries(this.errors).map(([field, e]) => {
      if (e instanceof typecasts.RequiredError) {
        ...
        messages[field] = this[CORE].messageCodeResolver.format(`form.field-error.${e.code}.${field}`, e.params);
      }
      else {
